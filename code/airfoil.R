...
#######################################################################
# Data Cleaning #######################################################
airfoil_data <- read.delim("../data/airfoil_self_noise.dat", header = FALSE)
...
airfoil_data <- airfoil_data %>% 
  rename(
    ...
    "Suction.Side.Displacement.Thickness" = 5, 
    "Scaled.Sound.Pressure.Level" = 6
  ) %>% 
  mutate_at(c(1:5), function(x) {(x - min(x)) / (max(x) - min(x))}) %>% 
  as.matrix()
